...
run: python3 profiling_performance_gpt2.py \
...
try:
    from torch_optimizer import QHM, AdamP
except ImportError:
    QHM = AdamP = None
...
def train_benchmark(model, loader, dataset, collator_stats, cfg, device, steps=500):
    ...
    opt_name = cfg.get("optimizer").lower()

    if opt_name == "adam8bit":
        opt = Adam8bit(model.parameters(), lr=3e-4)

    elif opt_name == "adafactor":
        opt = Adafactor(model.parameters(), scale_parameter=True, relative_step=True, warmup_init=True, lr=None)

    elif opt_name == "qhm":
        opt = QHM(model.parameters(), lr=3e-4, momentum=0.9, nu=1.0)

    elif opt_name == "adamp":
        opt = AdamP(model.parameters(), lr=3e-4)

    elif opt_name == "sophiag":
        opt = SophiaG(model.parameters(), lr=3e-4, rho=0.04, weight_decay=0.0)

    elif opt_name == "adamw":
            opt = torch.optim.AdamW(model.parameters(), lr=3e-4, foreach=True)
    
    elif opt_name == "lion8bit":
        opt = Lion8bit(model.parameters(), lr=3e-4)
    
    elif opt_name == "fusedadam":
        opt = FusedAdam(model.parameters(), lr=3e-4, betas=(0.9, 0.999), eps=1e-8)

    else:
        raise ValueError("No proper optimizer was specified")

    print(f'the optimiser being used is; {opt_name}')
    print(opt)
    ...
    return {
        ...
        "tokens_per_sec": token_total / total_time if total_time > 0 else -1,
...
def main():
    ...
    fieldnames = sorted(set().union(*[cfg.keys() for cfg in exp_matrix]) | {
        "steps", "total_time", "tokens_per_sec", "max_vram", "max_cpu_ram",
